...
class App extends Component {
  ...
  onClickVisit = id => {
    ...
    const visitedCountries = countryList.map(eachCountry => {
  ...
  onDeleteItem = id => {
    const {countryList} = this.state
    const updatedList = countryList.map(eachCountry => {
      if (eachCountry.id === id) {
        ...
        <div className="card-container">
          ...
          {updatedList.length > 0 ? (
            ...
            <div className="no-country">
              <p className="empty">No Countries Visited Yet</p>
            </div>
          )}
        </div>
      </div>
